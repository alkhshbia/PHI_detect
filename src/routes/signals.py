...
def get_signal_stats():
    ...
    try:
        ...
        # Additional filters to match get_processed_signals
        status_filter = request.args.get('status', 'all')
        pinned_filter = request.args.get('pinned_filter', 'all')
        ...
        # Filter by signal flag if requested (legacy parameter, kept for compatibility)
...
def export_signals_csv():
    ...
    try:
        ...
        for signal in signals:
            ...
            title = raw_article.title if raw_article and raw_article.title else "article"
            title_slug = create_url_slug(title)
            eios_url = f"https://eios.who.int/portal/monitoring/article-detail/{signal.rss_item_id}/{title_slug}/full-article"
