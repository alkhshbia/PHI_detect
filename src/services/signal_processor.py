...
class LocationReferenceManager:
    """
    Manages official location reference data for efficient country and admin1 matching.
    Loads data once at initialization to avoid token waste in LLM prompts.
    """
    
    def __init__(self, reference_dir: str = "reference"):
        """
        Initialize location reference manager with CSV data.
        
        Args:
            reference_dir: Directory containing continent_country.csv and country_admin1.csv
        """
        self.countries_by_continent: Dict[str, Set[str]] = {}
        self.all_countries: Set[str] = set()
        self.admin1_by_country: Dict[str, Set[str]] = {}
        self.countries_with_admin1: Set[str] = set()
        
        # Load reference data
        self._load_continent_country_data(reference_dir)
        self._load_country_admin1_data(reference_dir)
        
        logger.info(f"Loaded {len(self.all_countries)} countries and {sum(len(admin1s) for admin1s in self.admin1_by_country.values())} admin1 subdivisions")
    
    def _load_continent_country_data(self, reference_dir: str):
        """Load continent-country mapping from CSV."""
        file_path = os.path.join(reference_dir, "continent_country.csv")
        try:
            with open(file_path, 'r', encoding='utf-8-sig') as f:
                ...
                for row in reader:
                    ...
                    if continent not in self.countries_by_continent:
                        self.countries_by_continent[continent] = set()
                    
                    self.countries_by_continent[continent].add(country)
                    self.all_countries.add(country)
                    
        except Exception as e:
            logger.error(f"Error loading continent-country data: {e}")
    
    def _load_country_admin1_data(self, reference_dir: str):
        """Load country-admin1 mapping from CSV."""
        file_path = os.path.join(reference_dir, "country_admin1.csv")
        try:
            with open(file_path, 'r', encoding='utf-8-sig') as f:
                reader = csv.DictReader(f)
                for row in reader:
                    country = row['admin0'].strip()
                    admin1 = row['admin1'].strip()
                    
                    if country not in self.admin1_by_country:
                        self.admin1_by_country[country] = set()
                    
                    self.admin1_by_country[country].add(admin1)
                    self.countries_with_admin1.add(country)
                    
        except Exception as e:
            logger.error(f"Error loading country-admin1 data: {e}")
    
    def get_official_countries(self) -> List[str]:
        """Get list of all official country names."""
        return sorted(list(self.all_countries))
    
    def has_admin1_data(self, country: str) -> bool:
        """Check if a country has admin1 subdivision data."""
        return country in self.countries_with_admin1
    
    def get_admin1_for_country(self, country: str) -> List[str]:
        """Get admin1 subdivisions for a specific country."""
        return sorted(list(self.admin1_by_country.get(country, set())))
    
    def get_countries_by_continent(self, continent: str) -> List[str]:
        """Get countries in a specific continent."""
        return sorted(list(self.countries_by_continent.get(continent, set())))
    
    def find_matching_countries(self, text: str, threshold: int = 3) -> List[str]:
        """
        Find countries mentioned in text using fuzzy matching.
        
        Args:
            text: Text to search for country mentions
            threshold: Minimum character length for partial matching
            
        Returns:
            List of matching official country names
        """
        ...
        for country in self.all_countries:
            ...
            if len(country) >= threshold:
                words = country_lower.split()
                for word in words:
                    if len(word) >= threshold and word in text_lower:
                        matches.append(country)
                        break
        
        return list(set(matches))  # Remove duplicates
    
    def generate_location_context_for_prompt(self, suspected_countries: List[str] = None) -> str:
        """
        Generate location context for LLM prompt without including full lists.
        
        Args:
            suspected_countries: Countries suspected to be relevant (optional)
            
        Returns:
            String with location guidance for the prompt
        """
        context = "Use official country names from the UN/WHO standard list. "
        
        if suspected_countries:
            # Include admin1 info for suspected countries
            admin1_countries = [c for c in suspected_countries if self.has_admin1_data(c)]
            if admin1_countries:
                context += f"For these countries, include admin1/state/province if identifiable: {', '.join(admin1_countries)}. "
        
        context += "If multiple countries are mentioned, include all relevant ones."
        return context
...
class ArticleProcessor:
    def __init__(self):
        ...
        * ``AI_MODEL`` – optional, model name to use. Defaults to ``gpt-4``.
        ...
        default_prompt = (
            ...
            "Rules:\n"
            "- Use official UN/WHO country names only. Include admin1/state/province when clearly identifiable.\n"
            ...
            if best_match:
                ...
                if self.location_manager.has_admin1_data(best_match):
                    ...
                    if found_admin1:
                        ...
                        enhanced_countries.append(best_match)
                else:
                    enhanced_countries.append(best_match)
            else:
    ...
    def process_article(self, raw_article: RawArticle, is_pinned: bool = False) -> ProcessedArticle:
        ...
        # Pre-analyze text to find potential countries for location context
        suspected_countries = self.location_manager.find_matching_countries(raw_article.combined_text)
        location_context = self.location_manager.generate_location_context_for_prompt(suspected_countries)
        
        # Perform a single AI call for risk evaluation with location context
        risk_response = self.ask_ai(
            self.risk_evaluation_prompt.format(
                text=raw_article.combined_text,
